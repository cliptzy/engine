...
10. **Penghentian Komponen Tombol Lama (Deprecation)**: Sejak Flet 0.80.0+, komponen `ft.ElevatedButton`, `ft.OutlinedButton`, dan `ft.TextButton` telah **usang (deprecated)**. Anda WAJIB menggantinya dengan `ft.Button` biasa dan menggunakan parameter `style=ft.ButtonStyle(...)` atau varian button standard terbaru.
11. **Dilarang Memanggil `self.update()` di dalam `__init__`**: Pada komponen _UserControl_ atau subclass Flet, pemanggilan `self.update()` (baik langsung maupun tidak langsung via method lain seperti `_refresh_list()`) di dalam konstruktor `__init__` akan menyebabkan `RuntimeError: Control must be added to the page first`. PERHATIAN: Memeriksa `if self.page:` juga akan memicu error yang sama karena mengakses properti `.page` melempar `RuntimeError` jika belum di-mount. Anda harus membungkusnya dengan `try: self.update() except RuntimeError: pass` atau memanggilnya hanya di event handler / `did_mount`.
