...
def install_build_dependencies(target_platform):
    if os.environ.get("GITHUB_ACTIONS") == "true":
        print(
            "[INFO] Running in GitHub Actions. Skipping dependency installation as runners have pre-installed tools."
        )
        return

    print(
        f"[INFO] Checking and installing build dependencies for platform: {target_platform}..."
    )
    if target_platform == "linux":
        if shutil.which("apt-get"):
            print(
                "[INFO] Debian/Ubuntu system detected. Installing dependencies via apt..."
            )
            try:
                ...
                print(
                    "[WARNING] Please run the following command manually to install dependencies:"
                )
                print(
            ...
            print(
                "[WARNING] Package manager 'apt-get' not found. Please install building tools (clang, cmake, ninja-build, pkg-config, gtk3-dev, lzma-dev) manually."
            )

    elif target_platform == "macos":
        if shutil.which("brew"):
            print(
                "[INFO] macOS system detected. Installing dependencies via Homebrew..."
            )
            try:
            ...
            print(
                "[WARNING] Homebrew ('brew') not found. Please install 'cmake' and 'ninja' manually."
            )

    elif target_platform == "windows":
        print("[INFO] Windows system detected.")
        print(
            "[INFO] Visual Studio 2022 Build Tools (with C++ Desktop workload) is required."
        )
        ...
        if shutil.which("winget"):
            try:
                ...
                print(
                    f"[WARNING] Failed to automatically install Visual Studio Build Tools: {e}"
                )
                print(
                    "[WARNING] Please install 'Desktop development with C++' workload using Visual Studio Installer manually."
                )
        else:
            print(
                "[WARNING] 'winget' not found. Please download and install Visual Studio with C++ desktop workload manually."
            )
...
def main():
    ...
    if not flet_bin:
        print(
            "[ERROR] Flet executable not found! Make sure 'flet' is installed in the active environment."
        )
        sys.exit(1)
