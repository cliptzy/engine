...
class BrainrotScriptGenerator:
    ...
    def generate_script(
        ...
        prompt = f"""
You are an expert comedy writer specializing in TikTok/YouTube Shorts "Brainrot" content.
...
4. Output MUST be a valid JSON object containing a "script" array.
...
{{
  "script": [
    {{"speaker": "{character_1}", "text": "Dialog baris pertama..."}},
    {{"speaker": "{character_2}", "text": "Balasan dari karakter 2..."}}
  ]
}}
```
"""
        log.info(f"[Brainrot] Meminta script ke AI Provider: {provider_name.upper()}...")
        if event_hook:
        ...
        try:
            ...
            def extract_array(text: str) -> list:
                ...
                match_md = re.search(r"```json\s*(\[\s*\{.*?\}\s*\])\s*```", text, re.DOTALL | re.IGNORECASE)
                if match_md:
            ...
            try:
                ...
                if isinstance(parsed_raw, dict):
                    if "script" in parsed_raw and isinstance(parsed_raw["script"], list):
