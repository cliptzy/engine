...
class AppConfig:
    ...
    def update_from_dict(self, data: dict) -> None:
        ...
        if (
            "subtitle_border_style" in data
            and data["subtitle_border_style"] is not None
        ):
        ...
        if "yt_client_id" in data:
            self.youtube.client_id = data["yt_client_id"]
        if "yt_client_secret" in data:
            self.youtube.client_secret = data["yt_client_secret"]
        if "yt_visibility" in data:
            self.youtube.visibility = data["yt_visibility"]
        if "yt_auto_upload" in data:
            self.youtube.auto_upload = bool(data["yt_auto_upload"])

        if "tt_session" in data:
            self.tiktok.session = data["tt_session"]
        if "tt_privacy" in data:
            self.tiktok.privacy = data["tt_privacy"]
        if "tt_auto_upload" in data:
            self.tiktok.auto_upload = bool(data["tt_auto_upload"])

        if "ig_business_id" in data:
            self.instagram.business_id = data["ig_business_id"]
        if "ig_access_token" in data:
            self.instagram.access_token = data["ig_access_token"]
        if "ig_session" in data:
            self.instagram.session = data["ig_session"]
        if "ig_auto_upload" in data:
            self.instagram.auto_upload = bool(data["ig_auto_upload"])
    ...
    def get_files_to_sync(self) -> list:
        ...
        if self.tiktok.session:
            files.append(self.tiktok.session)
        if self.youtube.session:
            files.append(self.youtube.session)
        if self.instagram.session:
            files.append(self.instagram.session)
        return files
