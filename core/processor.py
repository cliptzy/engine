...
from core.processing.utils import cleanup_temp_files, run_command_with_logging
...
def process_single_clip(
    ...
    if min_dur > 0 and (end - start) < min_dur:
        ...
        if expansion_needed > 0:
            ...
            log.info(
                f"[Clip {index}] Durasi klip ({end - start:.1f}s) di bawah batas minimum ({min_dur}s). Menyesuaikan rentang ke ({new_start:.1f}s - {new_end:.1f}s)."
            )
            start = new_start
            end = new_end
    ...
    log.info(
        f"[Clip {index}] Processing segment ({int(start)}s - {int(end)}s, padding {config.padding}s)"
    )
    ...
    ydl_opts_fallback: dict[str, Any] = {
        ...
        "progress_hooks": [
            create_yt_dlp_progress_hook(event_hook, "[yt-dlp-fallback]")
        ],
    }
    ...
    try:
        if not os.path.exists(cropped_file):
            if source_url and os.path.isfile(source_url):
                ...
                try:
                    ...
                    log.info(
                        f"Retrying download with fallback format for clip {index}: {e}"
                    )
            ...
            if crop_mode == "center_face":
                ...
                log.info(
                    f"Detecting face keyframes for dynamic center crop in clip {index}..."
                )
                try:
                ...
                try:
                    ...
                    res_cx, res_cy, error_msg = get_dominant_face_normalized_center(
                        temp_file
                    )
                    if res_cx is None or res_cy is None:
                        log.info(
                            f"Clip {index}: No face detected ({error_msg}), falling back to full mode."
                        )
                        log.info(
                            f"Wajah tidak terdeteksi pada klip {index} ({error_msg}). Beralih ke mode Full (fallback)."
                        )
                        crop_mode = "full"
                    else:
            ...
            if config.ai.use_emotion_detection:
                ...
                visual_emotions = analyze_video_emotions(
                    temp_file, cx_norm, cy_norm, interval_sec=1.0, crop_mode=crop_mode
                )
            else:
                log.info(
            ...
            if getattr(config.ai, "use_emotion_detection", True):
                try:
                    ...
                    audio_emotions = analyze_audio_emotions(
                        temp_file, chunk_duration=2.0, overlap=1.0
                    )
                except Exception as e:
                    log.warning(f"Gagal memuat audio analyzer: {e}")

            audio_emotion_file = os.path.join(
                config.job_dir, f"audio_emotion_{index}.json"
            )
            try:
                with open(audio_emotion_file, "w", encoding="utf-8") as f:
                    json.dump(audio_emotions, f)
            except Exception as e:
                pass
        ...
        subtitle_generated, transcript_text, words_data = generate_subtitle(
            cropped_file,
            subtitle_file,
            config.subtitle.whisper_model,
        ...
        if transcript_text:
            log.info(
                f"Generating metadata (Title, Desc, Highlight) via AI for clip {index}..."
            )
            if callable(event_hook):
                try:
            ...
            if metadata:
                ...
                if enriched_transcript and isinstance(enriched_transcript, list):
                    ...
                    write_enriched_ass_file(
                        enriched_transcript, subtitle_file, event_hook=event_hook
                    )
        ...
        if phase1_only:
            log.info(
                f"Phase 1 complete for clip {index}. Skipping rendering (Phase 2)."
            )
            if callable(event_hook):
                try:
...
def render_single_clip(
    ...
    cropped_file = (
        nosub_matches[0]
        if nosub_matches
        else os.path.join(job_dir, f"clip_{index}_nosub.mp4")
    )
