...
class SupabaseSyncManager:
    ...
    def login_with_google(self) -> bool:
        ...
        try:
            ...
            if auth_code[0]:
                ...
                session_res = self.client.auth.exchange_code_for_session({"auth_code": auth_code[0]}) # type: ignore
                if session_res and session_res.user:
                    ...
                    session_data = {
                        "access_token": session_res.session.access_token if session_res.session else None,
                        "refresh_token": session_res.session.refresh_token if session_res.session else None
                    }
    ...
    def backup_all(self, on_progress: Optional[Any] = None) -> Dict[str, Any]:
        ...
        """
        if not self.client or not self.user:
            return {"success": False, "message": "Belum login. Silakan login terlebih dahulu."}
                ...
                if isinstance(channels_data, list):
                    ...
                    for ch in channels_data:
                        ...
                        if ch_id:
                            ...
                            if on_progress:
                                on_progress(f"Download {ch_filename}...", current, total)
                            ok = self.download_file(ch_filename, ch_filename)
                            results[ch_filename] = ok
                            current += 1
            except Exception as e_channels:
                log.warning(f"Gagal memulihkan katalog channel individual: {e_channels}")
