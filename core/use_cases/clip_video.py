...
class ClipVideoUseCase:
    ...
    def execute(
        ...
        max_clips = (
            payload.get("max_clips") if payload.get("max_clips") is not None else 10
        )
        ...
        config.subtitle.delay = (
            subtitle_delay / 1000.0 if subtitle_delay > 10 else subtitle_delay
        )
        ...
        if isinstance(picked, list) and len(picked) > 0:
            ...
            if start_s is None and end_s is None:
                ...
                log.info(
                    f"Menggunakan rentang waktu kustom: memproses keseluruhan video (0s - {end_s}s)"
                )

            if start_s is None or end_s is None:
                raise ValueError(
                    "Waktu Mulai dan Selesai harus diisi, atau kosongkan keduanya untuk memproses seluruh video"
                )
            if end_s <= start_s:
            ...
            if not segments:
                raise RuntimeError(
                    "Data Most Replayed / Heatmap tidak ditemukan untuk video ini"
                )
        ...
        if (
            ...
            try:
                ...
                try:
                    from core.utils import get_preview_data, read_json, write_json
                    ...
                    if combined_texts:
                        ...
                        clip_text = (
                            "Ini adalah kompilasi video panjang dari beberapa momen. Berikut ringkasannya:\n"
                            + "\n\n".join(combined_texts)
                        )
    ...
    def generate_subtitle_preview_sample(self, payload: Dict[str, Any]) -> str:
        ...
        config.subtitle.location = subtitle_location
        config.subtitle.delay = (
            subtitle_delay / 1000.0
            if subtitle_delay > 10 or subtitle_delay < -10
            else subtitle_delay
        )
        ...
        log.info(
            f"[PREVIEW] Memproses sampel 10 detik ({int(start)}s - {int(start + 10)}s) dengan Subtitle Delay: {subtitle_delay}ms..."
        )
