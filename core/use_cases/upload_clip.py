...
class UploadClipUseCase:
    ...
    def execute(self, video_path: Path, title: str, description: str, tags: List[str], publish_at: Optional[datetime] = None) -> bool:
        """
        ...
        if self.reporter:
            if result.success:
                ...
                self.reporter.on_error(f"Upload failed for {video_path.name}: {result.error_msg}")
