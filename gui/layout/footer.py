...
class StatusBar(ft.Container):
    def __init__(self, **kwargs: Any):
        ...
        # Left Side (Status)
        self.cookies_status = ft.Text("🍪 Cookies: Loading...", size=11, color=ft.Colors.ON_SURFACE_VARIANT)
        self.ffmpeg_status = ft.Text("🎥 FFmpeg: Loading...", size=11, color=ft.Colors.ON_SURFACE_VARIANT)
        self.deno_status = ft.Text("🦕 Deno: Loading...", size=11, color=ft.Colors.ON_SURFACE_VARIANT)
        ...
        self.cpu_ram_status = ft.Text("💻 CPU: 0% | RAM: 0%", size=11, color=ft.Colors.ON_SURFACE_VARIANT)
        self.network_status = ft.Text("🌐 0 KB/s", size=11, color=ft.Colors.ON_SURFACE_VARIANT)
        self.clock_status = ft.Text("🕒 00:00:00", size=11, color=ft.Colors.ON_SURFACE_VARIANT, weight=ft.FontWeight.BOLD)

        self.progress_bar = ft.ProgressBar(width=100, color=ft.Colors.PRIMARY, bgcolor=ft.Colors.SURFACE, value=0.0)
        self.progress_bar.visible = False

        self.progress_label = ft.Text(size=11, color=ft.Colors.ON_SURFACE_VARIANT, visible=False)
    ...
    def _check_dependencies(self):
        ...
        self.cookies_status.value = "Cookies: OK" if has_cookies else "Cookies: Not Found"
        self.cookies_status.color = ft.Colors.GREEN_400 if has_cookies else ft.Colors.ERROR

        self.ffmpeg_status.value = "FFmpeg: OK" if has_ffmpeg else "FFmpeg: Not Found"
        self.ffmpeg_status.color = ft.Colors.GREEN_400 if has_ffmpeg else ft.Colors.ERROR
