...
class CreatorHubView(ft.Container):
    def __init__(
        self, on_video_select: Optional[Callable[[str], Any]] = None, **kwargs: Any
    ):
    ...
    def _build_ui(self):
        ...
        self.channels_page = ft.Column(
            ...
            controls=cast(
                ...
                [
                    ft.Text(
                        "Pilih YouTuber untuk menjelajahi daftar video Upload & Live Stream:",
        ...
        self.active_channel_title = ft.Text(
            "Channel: -",
            size=16,
            weight=ft.FontWeight.BOLD,
        ...
        self.tab_group = ft.RadioGroup(
            value="upload",
            content=ft.Row(
                controls=cast(list[ft.Control], [self.radio_uploads, self.radio_live])
            ),
        )
        ...
        self.btn_prev_page = ft.Button(
            "◀ Prev", on_click=self.on_prev_page, disabled=True
        )
        self.page_info_label = ft.Text("Halaman 1 dari 1", weight=ft.FontWeight.BOLD)
        self.btn_next_page = ft.Button(
            "Next ▶", on_click=self.on_next_page, disabled=True
        )
                ...
                [
                    ft.Row(
                        controls=cast(
                            ...
                            [
                                ft.TextButton(
                                    "⬅ Kembali ke Daftar YouTuber",
                                    on_click=self.show_channels_grid,
                                ),
        ...
        self.content = ft.Column(
            expand=True,
            controls=cast(
                list[ft.Control], [top_bar, self.channels_page, self.videos_page]
            ),
        )
                    ...
                    [
                        ...
                        ft.Column(
                            ...
                            controls=cast(
                                ...
                                [
                                    ft.Text(
                                        str(c_name), weight=ft.FontWeight.BOLD, size=14
                                    ),
                                    ft.Text(
                                        f"{handle} • {c_subs}",
                                        size=12,
                        ...
                        ft.IconButton(
                            ...
                            on_click=lambda e, cid=c_id, cname=c_name: (
                                self.delete_channel(e, cid, cname)
                            ),
                        ),
                    ],
                ),
    ...
    def on_add_channel(self, e: Any):
        ...
        async def scrape_worker():
            ...
            try:
                ...
                ch_data = await asyncio.to_thread(
                    channel_manager.add_channel_by_url_or_handle, query
                )  # type: ignore
                if ch_data:
    ...
    def delete_channel(
        self, e: Any, channel_id: Optional[str], channel_name: str
    ) -> None:
        if not channel_id:
            return
        from core.channel_manager import channel_manager
    ...
    def open_channel_catalog(
        self, channel_id: Optional[str], channel_name: str, channel_subs: str = ""
    ):
        if not channel_id:
            return
    ...
    def load_video_catalog(self):
        ...
        for v in videos:
            ...
            card = VideoCard(
                video_info=vi, on_click=self._create_video_click_handler(url)
            )
