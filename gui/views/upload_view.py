...
class UploadView(ft.Column):
    def __init__(self, page: ft.Page):
        ...
        self.refresh_projects_btn = ft.IconButton(
            icon=ft.Icons.REFRESH, on_click=self.load_projects, tooltip="Refresh"
        )  # type: ignore
        self.open_project_dir_btn = ft.IconButton(
            icon=ft.Icons.FOLDER_OPEN,
            on_click=self.open_project_dir,
            tooltip="Open Folder",
        )
        ...
        self.video_placeholder = ft.Container(
            content=ft.Column(
                controls=[
                    ...
                    ft.Text(
                        "Pilih klip untuk memutar video",
                        color=ft.Colors.BLUE_GREY_400,
                        size=14,
                    ),
                ],
        ...
        self.meta_tags = ft.TextField(
            label="Hashtags (pisahkan dengan spasi)",
        ...
        self.btn_ai_meta = ft.Button(
            "Generate dengan AI",
            icon=ft.Icons.AUTO_AWESOME,
            on_click=self.generate_ai_metadata,
        )

        # --- Upload Settings ---
        self.platform_yt = ft.Checkbox(
            label="YouTube Shorts", value=config.youtube.auto_upload
        )
        self.platform_tt = ft.Checkbox(label="TikTok", value=config.tiktok.auto_upload)
        self.platform_ig = ft.Checkbox(
            label="Instagram Reels", value=config.instagram.auto_upload
        )
        ...
        self._page.overlay.extend(
            [self.schedule_date_picker, self.schedule_time_picker]
        )
        ...
        self.btn_pick_date = ft.TextButton(
            "Set Tanggal",
            icon=ft.Icons.CALENDAR_MONTH,
            on_click=lambda _: self._page.show_dialog(self.schedule_date_picker),
        )
        self.btn_pick_time = ft.TextButton(
            "Set Waktu", icon=ft.Icons.ACCESS_TIME, on_click=show_time
        )
        self.btn_clear_schedule = ft.IconButton(
            icon=ft.Icons.CLEAR, on_click=self.clear_schedule, tooltip="Reset Jadwal"
        )
        self.lbl_schedule_info = ft.Text(
            "Jadwal Mulai: Segera (+30 menit)", size=12, color=ft.Colors.BLUE_GREY_400
        )
        ...
        self.timeline_slider = ft.Slider(
            min=0, max=100, value=0, on_change=self.on_timeline_changed, expand=True
        )

        # --- Time Labels ---
        self.current_time_text = ft.Text(
            "00:00", size=12, color=ft.Colors.BLUE_GREY_400
        )
        ...
        right_panel = ft.Column(
            [self.video_container, self.media_controls_row], expand=2
        )
    ...
    def update_schedule_info(self) -> None:
        if self.schedule_date and self.schedule_time:
            ...
            self.lbl_schedule_info.value = (
                f"Jadwal Mulai: {self.schedule_date.strftime('%Y-%m-%d')} (00:00)"
            )
            ...
            self.lbl_schedule_info.value = (
                f"Jadwal Mulai: Hari ini {self.schedule_time.strftime('%H:%M')}"
            )
        else:
    ...
    def _create_video_player(self, path: str) -> ftv.Video:
        ...
        initial_vol = int(
            self.volume_slider.value if self.volume_slider.value is not None else 80
        )
        return ftv.Video(
            expand=True,
    ...
    def on_project_selected(self, e: Any) -> None:
        ...
        meta_files = glob.glob(
            os.path.join(self.current_project_dir, "metadata_*.json")
        )
        ...
        for idx in clip_indices:
            if idx == "merge":
                ...
                nosub_matches = glob.glob(
                    os.path.join(self.current_project_dir, f"clip_{idx}_*_nosub.mp4")
                )
                nosub_path = (
                    nosub_matches[0]
                    if nosub_matches
                    else os.path.join(self.current_project_dir, f"clip_{idx}_nosub.mp4")
                )
            ...
            chk = ft.Checkbox(
                value=False, data={"index": idx, "path": video_path, "status": status}
            )
            ...
            status_color = (
                ft.Colors.GREEN_400 if "Rendered" in status else ft.Colors.ORANGE_400
            )
    ...
    def load_clip_data(self, path: str, bname: str) -> None:
        ...
        if self.current_clip_index:
            meta_path = os.path.join(
                self.current_project_dir, f"metadata_{self.current_clip_index}.json"
            )
    ...
    def generate_ai_metadata(self, e: Any) -> None:
        ...
        if not transcript and not youtube_title:
            show_snackbar(
                self._page,
                "Tidak ada konteks video (transkrip/judul) untuk AI.",
                error=True,
            )
            return
        ...
        async def run_ai():
            ...
            app_state.append_log(
                "Memulai Generate Metadata AI (Upload & Distribution)..."
            )
    ...
    def handle_upload(self, e: Any) -> None:
        ...
        if not selected_clips:
            show_snackbar(
                self._page, "Pilih setidaknya satu klip untuk di-upload!", error=True
            )
            return

        platforms = []
        if self.platform_yt.value:
            platforms.append("YouTube Shorts")
        if self.platform_tt.value:
            platforms.append("TikTok")
        if self.platform_ig.value:
            platforms.append("Instagram Reels")

        if not platforms:
            show_snackbar(
                self._page, "Pilih setidaknya satu platform tujuan upload!", error=True
            )
            return
    ...
    def handle_render(self, e: Any) -> None:
        ...
        if not segments:
            ...
            app_state.append_log(
                "Tidak ada klip individual yang dipilih untuk dirender."
            )
            return
        ...
        class FletProgressReporter:
            ...
            def on_progress(self, label: str, current: int, total: int) -> None:
                if label == "total_targets":
                    ...
                    self.view.upload_status_text.value = (
                        f"{label}: klip {current}/{total}"
                    )
                try:
                    self.view.update()
                except Exception:
        ...
        async def render_worker():
            ...
            try:
                ...
                res = await asyncio.to_thread(
                    controller.execute_rendering, payload, check_cancelled
                )
