...
def generate_analyzer_debug(input_file: str, output_file: str, event_hook: Optional[Callable] = None) -> bool:
    if not os.path.exists(input_file):
        log.error("Input file not found.")
        return False
        
    try:
        from core.subtitle import get_whisper_model, _transcribe_with_language_sync, format_ass_time
        ...
        subprocess.run([
            ...
            "-i", input_file, "-vn", "-acodec", "pcm_s16le", "-ar", "16000", "-ac", "1", audio_wav
        ], check=True)
        
        # 2. Transcribe
        log.info("Mulai transkripsi Whisper untuk debug...")
        model = get_whisper_model("small")
        segments_gen = _transcribe_with_language_sync(model, audio_wav, word_timestamps=True, target_lang="id")
        ...
        for w in words_data:
            ...
            ass_events.append(f"Dialogue: 0,{s_ass},{e_ass},OverlayStyle,,0,0,0,,{text}\n")
        ...
        for i, ve in enumerate(visual_emotions):
            ...
            ass_events.append(f"Dialogue: 1,{s_ass},{e_ass},VisualStyle,,0,0,0,,{text}\n")
...
"""
        ...
        cmd = [
            ...
            "-i", input_file,
            "-vf", f"subtitles=filename='{ass_file_fwd}'",
            "-c:a", "copy",
            output_file
        ]
