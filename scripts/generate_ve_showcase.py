...
def generate_ve_showcase(input_file: str, output_file: str):
    ...
    for i, (emo, eff) in enumerate(effects_to_test):
        ...
        if v_type == "greenscreen":
            ...
            fc_parts.append(
                f"[{ve_idx}:v]scale={out_w}:-1,chromakey={key_col}:0.1:0.1,setpts=PTS-STARTPTS+{start_time}/TB{processed_v}"
            )
        elif v_type == "alpha":
            fc_parts.append(
                f"[{ve_idx}:v]scale={out_w}:-1,setpts=PTS-STARTPTS+{start_time}/TB{processed_v}"
            )
        else:
            fc_parts.append(
                f"[{ve_idx}:v]scale={out_w}:{out_h},setpts=PTS-STARTPTS+{start_time}/TB{processed_v}"
            )
        ...
        label_v = f"[lbl_v_{i}]"
        fc_parts.append(
            f"{processed_v}drawtext={font_arg}text='{label}':x=(w-text_w)/2:y=100:fontcolor=white:fontsize=36:box=1:boxcolor=black@0.6:boxborderw=10{label_v}"
        )

        next_v = f"[vout_{i + 1}]"
        fc_parts.append(
            f"{last_v}{label_v}overlay=x=(W-w)/2:y={pos_y}:enable='between(t,{start_time},{start_time + 4})':eof_action=pass{next_v}"
        )
        last_v = next_v
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "Usage: python generate_ve_showcase.py <input_video.mp4> <output_video.mp4>"
        )
        sys.exit(1)
